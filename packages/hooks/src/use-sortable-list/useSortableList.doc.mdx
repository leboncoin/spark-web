...
#### `getItemProps`
- **Type:** `<TElement extends HTMLElement = HTMLElement>(item: T, index: number) => SortableItemProps<TElement>`
- **Description:** Generic function that returns props to spread on a sortable item element. The element type `TElement` is inferred from usage or can be explicitly provided. Includes:
...
```tsx
<li
  {...getItemProps<HTMLLIElement>(item, index)}
