...
Use the `render` prop to replace with a custom element.
...
Use the `render` prop on `Avatar.User` to turn it into a link (or a button).
...
Use the `shape` prop to display a square avatar.
