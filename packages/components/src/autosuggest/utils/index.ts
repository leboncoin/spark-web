...
import { type ItemProps } from '../AutoSuggestItem'
import { type ItemTextProps } from '../AutoSuggestItemText'
...
export const getOrderedItems = (children: ReactNode, result: Option[] = []): Option[] => {
  Children.forEach(children, child => {
    ...
    if (getElementDisplayName(child) === 'AutoSuggest.Item') {
...
const findNestedItemText = (children: ReactNode): string => {
  ...
  for (const child of Children.toArray(children)) {
    if (isValidElement(child)) {
      ...
      if (getElementDisplayName(childElement) === 'AutoSuggest.ItemText') {
...
/**
 * If AutoSuggest.Item children:
 * - is a string, then the string is used.
 * - is JSX markup, then we look for AutoSuggest.ItemText to get its string value.
 */
