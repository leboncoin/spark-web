...
import { styles } from './AutoSuggestTrigger.styles'
...
export const Trigger = ({ className, children, ref: forwardedRef }: TriggerProps) => {
  ...
  const { state, refs, actions } = useAutoSuggest()
  const snap = useSnapshot(state)

  // Trigger compound elements
  const leadingIcon = findElement(children, 'AutoSuggest.LeadingIcon')
  const input = findElement(children, 'AutoSuggest.Input')
  const clearButton = findElement(children, 'AutoSuggest.ClearButton')
  const disclosure = findElement(children, 'AutoSuggest.Disclosure')
