...
<Callout kind="warning" marginY="large">
  <p>
    a11y: It is important that the trigger remain a button (with `type="button"`) and that it is
...
<Callout kind="info">
  **Note**: Unlike other form components, the `FormField.Label` is not automatically linked to the
  trigger button or dropzone via `aria-labelledby`. The accessible name of the button/dropzone
  remains their natural text content. The label is still useful for visual consistency, required
  indicators, and form structure.
</Callout>
