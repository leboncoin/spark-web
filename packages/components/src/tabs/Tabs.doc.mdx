...
You can add optional popovers to each tab trigger using the `renderMenu` prop on `Tabs.Trigger`. The popover is automatically positioned next to its trigger (right side for vertical orientation, bottom for horizontal). Press <Kbd>Shift</Kbd>+<Kbd>F10</Kbd> on a focused tab to open its associated popover.
...
- It is advised to use the `render` prop on each `Tabs.Trigger` with the appropriate heading tag (e.g. `render={<h2 />}`) according to your page structure.
...
- <Kbd>Shift</Kbd>+<Kbd>F10</Kbd>: Opens the popover associated with the focused trigger (if available).
