...
<ArgTypes of={RatingDisplay.Stars} />

### RatingDisplay.Value

<ArgTypes of={RatingDisplay.Value} />

### RatingDisplay.Count

<ArgTypes of={RatingDisplay.Count} />
...
### Custom star fill mode

Use `getFillMode` on `RatingDisplay.Stars` to override how each star is filled (`0`, `0.5`, `1`).

#### Rounded to integer (no half stars)

Always rounds the input rating to a whole number, then renders only full or empty stars.

<Canvas of={stories.RoundedToInteger} />

#### Rounded to 0.5 (always up)

Always rounds up to the nearest half, which differs from the default nearest-half algorithm.

<Canvas of={stories.RoundedHalfUp} />
...
Use the `render` prop so that the root renders as another element (e.g. a link): `render={<a href="..." />}`. The rating props are merged onto that element.

<Canvas of={stories.AsLink} />

## Accessibility

<A11yReport of="rating-display" />
