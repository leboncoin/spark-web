...
import { getNearestHalfDecimal, getSingleStarValue, getStarValue, splitAt } from './utils'
...
describe('getSingleStarValue', () => {
  it.each([
    { value: undefined, expected: 0 },
    { value: 0, expected: 0 },
    { value: 0.99, expected: 0 },
    { value: 1, expected: 0.5 },
    { value: 2.5, expected: 0.5 },
    { value: 3.99, expected: 0.5 },
    { value: 4, expected: 1 },
    { value: 5, expected: 1 },
  ])('should return the expected single star value for: $value', ({ value, expected }) => {
    expect(getSingleStarValue(value)).toEqual(expected)
  })
})
