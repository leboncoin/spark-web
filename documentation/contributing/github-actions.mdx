...
- **`SPARK_SA_TOKEN`**: GitHub token for repository access during releases
- **`VERCEL_TOKEN`**: Vercel authentication token for deployments
- **`VERCEL_ORG_ID`**: Vercel organization ID
- **`VERCEL_PROJECT_ID`**: Vercel project ID
- **`CODECOV_TOKEN`**: Codecov authentication token for coverage reporting
- **`GH_ENTERPRISE_INTERNAL_SSH_PRIVATE_KEY`**: SSH key for internal repository access
- **`GH_ENTERPRISE_INTERNAL_SSH_REPOSITORY_URL`**: Internal repository URL
