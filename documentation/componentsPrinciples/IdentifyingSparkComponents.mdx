...
<Meta title="Components Principles/Identifying Spark components" summary="Learn how to identify Spark components in the DOM using the data-spark-component attribute, with a bookmarklet tool for visual highlighting." />
...
```html
<button data-spark-component="button">Click me</button>
```
...
<Callout kind="warning" marginY="large">
  <p>iframes: the script is unable to detect components in iframes for now.</p>
</Callout>
...
<div className="sb-unstyled bg-support-container text-on-support-container p-xl w-full">
        ...
        el.href = 'javascript:' + encodeURIComponent(`
          !function(){
            ...
            e.textContent=\`
              .spark-overlay {
                position: absolute;
                background: var(--color-support, #094171);
                color: var(--color-on-support, #ffffff);
                padding: 4px 8px;
                font-size: 12px;
                font-weight: 700;
