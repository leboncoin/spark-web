...
<Meta title="Styling/Design Tokens" summary="Comprehensive reference for Spark's design tokens including color families, spacing, typography, shadows, and semantic theming system." />
    ...
    return (
      <Label
        ...
        className={cx(
          ...
          value === selectedValue ? 'bg-support text-on-support border-support' : 'border-support',
          'cursor-pointer',
...
<div className="sb-unstyled">
  <div className="gap-lg grid grid-cols-[repeat(auto-fit,minmax(400px,1fr))] flex-wrap">
    {fontSizes.map(fontSize => {
      return (
        <div>
          <p className={`${fontSize} bg-support-container text-on-support-container px-lg`}>
            Hello world.
          </p>
          <p>{fontSize}</p>
          <p className="text-body-2 opacity-dim-1">
            font-size: {getComputedCssVariable('font-size', fontSize)}, line-height:{' '}
            {getComputedCssVariable('line-height', fontSize)}, font-weight:{' '}
            {getComputedCssVariable('font-weight', fontSize)}
          </p>
        </div>
      )
    })}
